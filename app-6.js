...
async function autoRomanize(h, src, dst, area){
  ...
  try{
    ...
    // Use allorigins proxy — reliable CORS bypass for GitHub Pages
    const TARGET = 'https://api.anthropic.com/v1/messages';
    let r;
    // allorigins wraps the request on their server, bypassing CORS
    const proxyUrl = 'https://api.allorigins.win/raw?url=' + encodeURIComponent(TARGET);
    try {
      r = await fetch(proxyUrl, { method:'POST', headers, body });
    } catch(e) {
