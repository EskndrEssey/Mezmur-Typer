...
async function autoRomanize(h, src, dst, area){
  ...
  try{
    ...
    const proxies = [
      ...
      (url) => url // direct (works in some browsers)
    ];
    ...
    for(const makeUrl of proxies){
      try{
        r = await fetch(makeUrl(TARGET), { method:'POST', headers, body });
        if(r.ok || r.status===401) break; // got a real response
      }catch(e){ lastErr=e; r=null; }
    }

    if(!r) throw new Error('Could not reach API: ' + (lastErr?.message||'network error'));
