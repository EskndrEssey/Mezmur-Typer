...
function bindEditor(area, h){
  ...
  area.querySelectorAll('.romanize-btn').forEach(btn => {
    btn.addEventListener('click', () => autoRomanize(h, btn.dataset.src, btn.dataset.dst, area));
  });
...
async function autoRomanize(h, src, dst, area){
  ...
  try{
    ...
    // headers defined inline per request below

    const TARGET = 'https://api.anthropic.com/v1/messages';
    let r = null;

    // corsproxy.io passes POST bodies and custom headers correctly
    ...
    try {
      ...
      // Last resort: try without proxy (works on some networks)
      try {
        ...
        throw new Error('Cannot reach Anthropic API. Check internet connection.');
      }
    }
