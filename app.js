...
async function startSubmitFlow(hymn){
  ...
  // Ask volunteer for token if not yet provided this session
  if (!getVolToken()){
...
async function submitHymnToGitHub(){
  ...
  try{
    // Helper to append/update hymn in a file
    async function upsertInFile(filePath, hymnData){
      ...
      const msg=idx>=0?`Update hymn: ${title} (${hymnData.id})`:`Add hymn: ${title} (${hymnData.id})`;
      await ghPutFile(cfg, filePath, JSON.stringify(arr,null,2), sha, msg);
      return msg;
    }

    // Always submit to main category file
    const commitMsg = await upsertInFile(path, exported);

    // If subcategory is Mera AND main category isn't already Mera → also add to Mera.json
    if (hymn.subgroup==='Mera' && hymn.groupKey!=='Mera'){
      const meraPath=ghFilePath(cfg,'Mera');
      await upsertInFile(meraPath, exported);
    }
