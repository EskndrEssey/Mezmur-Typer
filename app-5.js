...
async function autoRomanize(h, src, dst, area){
  ...
  try{
    ...
    // Direct call with browser flag — works when CORS is allowed
    let r;
    try {
      ...
      try {
        r = await fetch('https://corsproxy.io/?' + encodeURIComponent('https://api.anthropic.com/v1/messages'), {
          method: 'POST', headers, body
        });
      } catch(proxyErr) {
        throw new Error('Network blocked. Make sure you have internet connection.');
      }
    }
